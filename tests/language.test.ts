...
test("planLanguage sends zh script subtags to Whisper as plain zh", () => {
	...
	for (const code of ["zh", "zh-Hans", "zh-CN", "zh-SG", "zh-Hant", "zh-TW", "zh-HK", "zh-MO"]) {
		assert.equal(planLanguage(code, true).whisper, "zh", `${code} should be sent as zh`);
	}
});

test("planLanguage steers Simplified for zh, zh-Hans, zh-CN and auto", () => {
	for (const code of ["auto", "zh", "zh-Hans", "zh-CN", "zh-SG", "ZH-hans"]) {
		assert.equal(planLanguage(code, true).steering, "simplified", `${code} should steer Simplified`);
	}
});
...
test("planLanguage steers Traditional when the language asks for it", () => {
	...
	for (const code of ["zh-Hant", "zh-TW", "zh-HK", "zh-MO", "ZH-hant"]) {
		assert.equal(planLanguage(code, true).steering, "traditional", `${code} should steer Traditional`);
	}
});

test("turning steering off leaves the script to Whisper, whichever direction was implied", () => {
	for (const code of ["auto", "zh", "zh-Hans", "zh-TW"]) {
		assert.equal(planLanguage(code, false).steering, undefined, `${code} with steering off`);
	}
});
...
test("planLanguage leaves every other language alone", () => {
	...
	// typo into a silently different language.
	for (const code of ["en", "ja", "yue", "en-US", "zh_CN", "chinese", "zh-Hans-CN"]) {
		const plan = planLanguage(code, true);
